...
const sum = (time) => {
    for(let i = 0; i < times.value.length; i++){
        if(times.value[i].id === time){
            return times.value[i].name
        }
    }
}
...
onBeforeMount(async () => {
    try{
        const response = await axios.get(`/api/payslip/details/${route.params.id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
        });
...
<template>
    ...
    <div class="container mb-4">
        <div class="bg-white p-3 rounded text-center">
            ...
            <h6 class="m-0 fw-semibold mt-1">{{ users.name }}</h6>
            <span class="fs-21 text-muted">Total Gaji : {{ format(total) }}</span>
        </div>

        
        <div class="bg-white col-12 p-2 rounded my-2">
            <div class="row mt-2 text-center">
                <div class="col">
                    <p class="fs-21 text-muted">Hadir</p>
                    <p class="fs-12 fw-semibold">{{ users.present }}</p>
                </div>
                <div class="col">
                    <p class="fs-21 text-muted">Tdk. Hadir</p>
                    <p class="fs-12 fw-semibold">{{ users.absence }}</p>
                </div>
                <div class="col">
                    <p class="fs-21 text-muted">Sakit</p>
                    <p class="fs-12 fw-semibold">{{ users.sick }}</p>
                </div>
                <div class="col">
                    <p class="fs-21 text-muted">Izin</p>
                    <p class="fs-12 fw-semibold">{{ users.leave }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white col-12 p-3 rounded my-2 cursor-pointer" v-for="data in datas" :key="data.id" data-bs-toggle="modal" data-bs-target="#settingModal" @click="editData(data)">
            ...
            <div class="col-10">
                ...
                <span class="text-muted fs-21">{{ format(data.val) }} / {{ sum(data.time_id) }}</span><br>
                <span class="fs-21">{{ format(data.value) }}</span>
            </div>
            <div class="col-2 p-2">
