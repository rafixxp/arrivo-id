...
const input = () => {
    cameraOut.value.click();
}

const previewOut = async (e) => {
...
const submit = async () => {
    try {
        if(!image){
            Toast.fire({
                icon: 'error',
                title: 'Silakan ambil foto clock in !'
            })
        }
        else{
...
<template>
    <div class="modal fade" id="clockOut" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true" ref="clockOutModal">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content mx-3">
                ...
                <div class="modal-body border-0 text-center px-4">
                    ...
                    <input type="file" accept="image/*" capture="user" ref="cameraOut" @change="previewOut" hidden>
                ...
                <div class="modal-footer border-0">
                    <button type="button" class="btn btn-dark w-100 fw-semibold" @click="submit"><span class="bi bi-arrow-down-left-circle-fill me-2"></span>Clock Out</button>
                </div>
            </div>
        </div>
    </div>
